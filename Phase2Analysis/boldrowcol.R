# Function to bold rows or cols in an xtable.  Based on code at:
# https://gist.github.com/floybix/452201
# But modified to not print and instead just return the modified table.
...
## library(xtable)
## x <- tail(iris)
## boldrowcol(xtable(x)) #each = "column"
## boldrowcol(xtable(x), each = "column", max = c(F,NA,NA,T,NA))
## boldrowcol(xtable(x), each = "row", max = FALSE)
## boldrowcol(xtable(x), x >= 6.5, type = "html")
